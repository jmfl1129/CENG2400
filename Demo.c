...
void l_check_position(void){
      if(qeiPositionL<=(2000-LEFT90)) {
        GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_2, 0x0);
        GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_3, 0x0);
        if(Run==1) Run=2;
        else if(Run==3) Run=0;
      }
      if(qeiPositionR<=(2000-LEFT90)) {
        GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_4, 0x0);
        GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_5, 0x0);
        if(Run==1) Run=3;
        else if(Run==2) Run=0;
...
void r_check_position(void){
      if(qeiPositionL<=(2000-RIGHT90)) {
        GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_2, 0x0);
        GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_3, 0x0);
        if(Run==1) Run=2;
        else if(Run==3) Run=0;
      }
      if(qeiPositionR<=(2000-RIGHT90)) {
        GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_4, 0x0);
        GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_5, 0x0);
        if(Run==1) Run=3;
        else if(Run==2) Run=0;
...
//     }
    NEWLINE;
        print("=============================================================="); NEWLINE;
        print("*               CUHK CSE CENG2400 (2018)                     *"); NEWLINE;
        print("*                                                            *"); NEWLINE;
        ...
        print("*  j - project (test 1)    k - project (test 2)              *"); NEWLINE;
        print("=============================================================="); NEWLINE;
    NEWLINE;

    NEWLINE1;
    print1("=============================================================="); NEWLINE1;
    print1("*               CUHK CSE CENG2400 (2018)                     *"); NEWLINE1;
    print1("*                                                            *"); NEWLINE1;
    ...
    print1("*  j - project (test 1)    k - project (test 2)              *"); NEWLINE1;
    print1("=============================================================="); NEWLINE1;
    NEWLINE1;
    ...
    while(1) {
      ...
      while(Run==0) {
        ...
        if(cin=='f'){
            ...
            //Rotate the IR sensor point toward the front direction
            ...
            //Smart car move forward until IR sensor detect obstacle in front of the car
            chkpos=0;             //do not check the steps
            forward();            //continue to forward
            while(getSensor()==1){      //forward until sensor detect 0
            }
            delayMS(150);        //move forward a little bit more
            stop();     //stop the car
            delayMS(100);       //delay 100 ms

            //Rotate Smart car 90 degree right
            ...
            //Smart car move forward until no wall in the left
            ...
            //Rotate Smart car 90 degree left
            ...
            chkpos=0;             //do not check the steps
            forward();
            delayMS(300);        //move forward a little bit more
            stop();     //stop the car
            delayMS(300);


            //Smart car move forward until no wall in the right
            cal_pos();            //rotate the sensor anti-clockwise to the reference point
            stepCnt=1;            //then rotate clockwise to the middle
            Rotate=2;             //Rotate=2 => step motor rotate clockwise; Rotate=0 => stop;
            phase=1;              //initial phase of step motor drive output signal
            while(stepCnt<DIV*10){} // DIV*5 equal to 180 degree
            Rotate=0;             //Rotate=0 stop the sensor rotation
